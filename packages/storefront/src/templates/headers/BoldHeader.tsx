      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} className={`px-4 py-2 text-sm font-bold uppercase tracking-wider transition-colors ${isActive(link.to) ? 'text-primary-400 bg-white/10 rounded-md' : 'text-gray-300 hover:text-white'}`}>{link.label}</Link>
            ))}
          </nav>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-800 bg-gray-950">
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} onClick={() => setMobileOpen(false)} className={`block px-3 py-2 text-sm font-bold uppercase ${isActive(link.to) ? 'text-primary-400' : 'text-gray-300'}`}>{link.label}</Link>
            ))}
