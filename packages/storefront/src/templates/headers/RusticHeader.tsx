      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-stone-200 dark:border-stone-700 bg-stone-100 dark:bg-stone-900">
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} onClick={() => setMobileOpen(false)} className={`block px-3 py-2 text-sm ${isActive(link.to) ? 'text-primary-700 bg-stone-200' : 'text-stone-600'}`}>{link.label}</Link>
            ))}
