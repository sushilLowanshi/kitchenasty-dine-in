      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${isActive(link.to) ? 'text-primary-600 bg-primary-50/80 dark:bg-primary-900/20' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100/80 dark:hover:bg-gray-800/50'}`}>{link.label}</Link>
            ))}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {/* Customer login / logout commented out for dine-in guest ordering */}
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-200/50 dark:border-gray-700/50 bg-white/90 dark:bg-gray-900/90 backdrop-blur-lg">
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} onClick={() => setMobileOpen(false)} className={`block px-3 py-2 rounded-lg text-sm ${isActive(link.to) ? 'text-primary-600 bg-primary-50' : 'text-gray-600'}`}>{link.label}</Link>
            ))}
