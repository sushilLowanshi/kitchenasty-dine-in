      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-amber-100 dark:border-gray-800 bg-amber-50 dark:bg-gray-900">
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} onClick={() => setMobileOpen(false)} className={`block px-3 py-2 rounded-full text-sm ${isActive(link.to) ? 'text-primary-700 bg-primary-100' : 'text-gray-600'}`}>{link.label}</Link>
            ))}
