      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-14">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} className={`text-sm transition-colors ${isActive(link.to) ? 'text-gray-900 dark:text-white' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}>{link.label}</Link>
            ))}
          </nav>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-100 dark:border-gray-800">
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} onClick={() => setMobileOpen(false)} className={`block px-3 py-2 text-sm ${isActive(link.to) ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>{link.label}</Link>
            ))}
