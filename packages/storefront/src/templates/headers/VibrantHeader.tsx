      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${isActive(link.to) ? 'bg-white/20 text-white' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}>{link.label}</Link>
            ))}
          </nav>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-white/20 bg-gradient-to-r from-primary-600 via-secondary-600 to-primary-600">
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.id} to={link.to} onClick={() => setMobileOpen(false)} className={`block px-3 py-2 text-sm ${isActive(link.to) ? 'text-white bg-white/20 rounded-md' : 'text-white/80'}`}>{link.label}</Link>
            ))}
